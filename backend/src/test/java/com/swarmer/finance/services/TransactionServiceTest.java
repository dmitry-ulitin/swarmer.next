...
public class TransactionServiceTest {
    ...
    private final Account account2 = new Account(null, group, "", "RUB",
            .0, false, LocalDateTime.now(), LocalDateTime.now());
    private final Category education = Category.builder().name("Education").parentId(1L).created(LocalDateTime.now())
    ...
    private final Category salary = Category.builder().name("Salary").parentId(2L).created(LocalDateTime.now())
            .updated(LocalDateTime.now()).build();
    private final Transaction income = Transaction.builder().owner(user).opdate(LocalDateTime.of(2022, 1, 1, 0, 0, 0))
            .debit(1000.0).recipient(account1).credit(1000.0).created(LocalDateTime.now()).updated(LocalDateTime.now())
            .build();
    private final Transaction transfer = Transaction.builder().owner(user).opdate(LocalDateTime.of(2022, 1, 5, 0, 0, 0))
            .account(account1).debit(500.0).recipient(account2).credit(50000.0).created(LocalDateTime.now())
            .updated(LocalDateTime.now()).build();
    private final Transaction expense = Transaction.builder().owner(user).opdate(LocalDateTime.of(2022, 1, 10, 0, 0, 0))
            .account(account2).debit(10000.0).credit(10000.0).category(education).created(LocalDateTime.now())
            .updated(LocalDateTime.now()).build();
    ...
    void testCreateTransaction() {
        var trx1 = new TransactionDto(null, user.getId(), LocalDateTime.of(2022, 1, 20, 0, 0, 0), TransactionType.EXPENSE,
                AccountDto.from(account2, user.getId(), null), 10000., null, 10000., education, null, "",
                "");
        var trx2 = new TransactionDto(null, user.getId(), LocalDateTime.of(2022, 1, 21, 0, 0, 0), TransactionType.EXPENSE,
                AccountDto.from(account2, user.getId(), null), 30000., null, 30000., education, null, "",
                "");
        var trx3 = new TransactionDto(null, user.getId(), LocalDateTime.of(2022, 1, 25, 0, 0, 0), TransactionType.INCOME,
    ...
    void testGetBalances() {
        ...
        var acc1Income = balances.stream().filter(b -> b.getAccountId() == null && account1.getId().equals(b.getRecipientId())).findFirst();
        ...
        var acc2Expense = balances.stream().filter(b -> account2.getId().equals(b.getAccountId()) && b.getRecipientId() == null).findFirst();
    ...
    void testUpdateTransaction() {
        ...
        var updated = new TransactionDto(dto.id(), dto.ownerId(), dto.opdate(), dto.type(),
                dto.account(), amount, dto.recipient(), amount, dto.category(), dto.currency(), "new party", "new details");
